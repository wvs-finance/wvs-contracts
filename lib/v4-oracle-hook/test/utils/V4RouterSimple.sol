...
import "lib/v4-periphery/lib/v4-core/lib/solmate/src/utils/SafeTransferLib.sol";
    ...
    function swapTo(address caller, PoolKey memory key, uint160 sqrtPriceX96) public {
        if (msg.sender != address(POOL_MANAGER_V4)) {
            ...
            while (!done) {
                ...
                (uint160 sqrtPriceX96Result, , ,) = POOL_MANAGER_V4.getSlot0(key.toId());
                done = sqrtPriceX96 == sqrtPriceX96Result;
            }
            return;
        }

        (uint160 sqrtPriceX96Before, , ,) = POOL_MANAGER_V4.getSlot0(key.toId());
