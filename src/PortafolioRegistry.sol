...
import {IListener} from "./login/Listener.sol";
...
contract PortafolioRegistry is  
    ...
    function on_log(IReactive.LogRecord memory _log, address _dst) external{
        if (_log._contract != LPM && _log.chain_id == block.chainid) revert("Invalid Log");
    ...
    function set_event_selector(bytes32 calldata _event_selector) external{
